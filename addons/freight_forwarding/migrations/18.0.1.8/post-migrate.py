...
def _fetch_candidate_roots(cr, main_table, rel_table, fk_col, extra_filter=""):
    """Untuk setiap record di main_table yang source_quotation_id masih kosong
    ...
    cr.execute(
        ...
        WHERE m.source_quotation_id IS NULL
        {extra_filter}
        GROUP BY rel.{fk_col}
        """
    )
...
def _fetch_resolved_roots(cr, main_table):
    """Seluruh record main_table (Booking) yang SUDAH punya source_quotation_id
    ...
    cr.execute(f"SELECT id, source_quotation_id FROM {main_table} WHERE source_quotation_id IS NOT NULL")
...
def _normalize_jobsheet_mirror_via_booking(cr, jobsheet_table, jobsheet_rel_table, jobsheet_fk_col, booking_roots):
    ...
    terisi) SENGAJA tidak punya source_quotation_id sendiri -- root-nya derived
    ...
    source_quotation_id Jobsheet TIDAK PERNAH ditulis di sini -- hanya
...
def _backfill_and_normalize(cr, label, main_table, rel_table, fk_col, extra_filter=""):
    ...
    for record_id, roots in rows:
        ...
        if root_id is None:
            ...
            continue
        cr.execute(
            f"UPDATE {main_table} SET source_quotation_id = %s WHERE id = %s",
            (root_id, record_id),
        )
    ...
    for record_id, conflicting_roots in conflicts:
        _logger.warning(
            ...
            "(root ids: %s) -- source_quotation_id DIBIARKAN kosong, tidak ada "
            "arbitrary canonicalization (dulu: MIN(sale_order_id)).",
            label, record_id, len(conflicting_roots), conflicting_roots,
        )

    _logger.info(
        "FF-73: %s -- source_quotation_id ter-resolve aman untuk %s record, "
...
def migrate(cr, version):
    """FF-73: backfill source_quotation_id untuk Booking/HBL/HAWB yang sudah ada,
    ...
    sebelum FF-73 ada), source_quotation_id DIBIARKAN kosong -- tidak ada
    pilihan arbitrer -- dan di-log sebagai WARNING supaya bisa ditelusuri
    manual. Kalau sale_order_ids kosong, source_quotation_id juga dibiarkan
    ...
    source_quotation_id DIBIARKAN kosong secara sengaja — root-nya didapat
    ...
    source_quotation_id baru dibuat oleh _auto_init setelah pre-migrate.

    Migration ini idempotent: memanggilnya berkali-kali tidak mengubah hasil
    (WHERE source_quotation_id IS NULL mencegah record yang sudah ter-resolve
    diproses ulang, dan mirror INSERT memakai NOT EXISTS).

    Follow-up gap fix: Jobsheet Export lewat Booking (booking_id terisi)
    sengaja TIDAK diberi source_quotation_id sendiri (root-nya derived dari
    ...
    # Jobsheet Export lewat Booking (booking_id terisi): source_quotation_id
    ...
    _normalize_jobsheet_mirror_via_booking(
        cr, "freight_sea_job", "freight_sea_job_sale_order_rel", "freight_sea_job_id",
        _fetch_resolved_roots(cr, "freight_sea_booking"),
    )
    _normalize_jobsheet_mirror_via_booking(
        cr, "freight_air_job", "freight_air_job_sale_order_rel", "freight_air_job_id",
        _fetch_resolved_roots(cr, "freight_air_booking"),
    )

    # Job (Sea) & Job (Air): backfill HANYA yang TIDAK punya booking_id
    # (flow direct-import) -- yang punya booking_id sengaja dibiarkan kosong
    # (source_quotation_id-nya), sudah ditangani di atas untuk mirror-nya.
    for label, main_table, rel_table, fk_col in [
        ("freight.sea.job", "freight_sea_job", "freight_sea_job_sale_order_rel", "freight_sea_job_id"),
        ("freight.air.job", "freight_air_job", "freight_air_job_sale_order_rel", "freight_air_job_id"),
    ]:
